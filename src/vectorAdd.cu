...
int main() {
  ...
  printf("Enter vector size: "); // For 100% occupancy, enter 1024 * 1024 * 400
                                 // = 419,430,400
  scanf("%d", &n);
  printf("Enter block size: "); // For 100% occupancy, enter 256
  scanf("%d", &block_size);
  ...
  int numBlocks = (n + block_size - 1) /
                  block_size; // Calculate the number of blocks for the kernel
  ...
  cudaCheckError(
      cudaMemcpy(h_c, d_c, bytes,
                 cudaMemcpyDeviceToHost)); // Copy the result back to the host
